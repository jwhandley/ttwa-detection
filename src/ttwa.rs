...
impl TravelToWorkAreas {
    ...
    fn flow_from_area(&self, area_index: &usize) -> i32 {
        ...
        area_nodes
            .iter()
            .map(|&node| self.flow_from_node[node])
            .sum()
    }
    ...
    fn self_containment_of_area(&self, area_index: &usize) -> f64 {
        ...
        let self_containment: i32 = area_nodes
            .iter()
            .map(|&i| {
                area_nodes
                    .iter()
                    .map(|&j| self.adjacency_matrix[(i, j)])
                    .sum::<i32>()
            })
            .sum();
    ...
    fn x_equation(&self, area_index: &usize) -> f64 {
        ...
        if (size > MIN_SIZE) && (self_containment > TARGET_CONTAINMENT) {
            ...
            (size - MIN_SIZE) / (TARGET_SIZE - MIN_SIZE)
    ...
    pub fn fit_travel_to_work_areas(&mut self) {
        ...
        for area in self.areas.keys() {
            self.self_containment
                .insert(*area, self.self_containment_of_area(area));
        }
        ...
        loop {
            ...
            for node in worst_area {
                ...
                self.self_containment
                    .insert(best_fit_area, self.self_containment_of_area(&best_fit_area));
